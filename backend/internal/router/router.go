...
func Setup(db *gorm.DB, cfg *config.Config) (*fiber.App, *services.OLTScheduler, *services.RouterScheduler) {
	...
	visitingSvc := services.NewVisitingService(visitRepo, activitySvc)
	dashboardSvc := services.NewDashboardService(routerRepo, pppoeRepo, iaRepo, packageRepo, profileMappingRepo, subRepo, billRepo, expenseRepo, activityRepo, visitRepo, db)
	...
	dashH := handlers.NewDashboardHandler(dashboardSvc, activitySvc, roleRepo)
	...
	billH := handlers.NewBillHandler(billingSvc, roleRepo)
	...
	visitH := handlers.NewVisitHandler(visitingSvc, billRepo, roleRepo)
	reportH := handlers.NewReportHandler(collectionReportRepo, roleRepo)
	...
	bills.Get("/account-due", perm("billing", "view"), billH.GetAccountDue)
	bills.Post("/generate", perm("billing", "create"), billH.GenerateBills)
	bills.Post("/collect", perm("billing", "update"), billH.CollectPayment)
	...
	// Visiting module
	visits := api.Group("/visits", jwtAuth)
	visits.Get("/pending-customers", perm("visiting", "view"), visitH.PendingCustomers)
	visits.Get("/today",             perm("visiting", "view"), visitH.Today)
	visits.Get("/",                  perm("visiting", "view"), visitH.List)
	visits.Post("/",                 perm("visiting", "create"), visitH.Create)
	visits.Get("/:id",               perm("visiting", "view"), visitH.Get)
	visits.Put("/:id",               perm("visiting", "update"), visitH.Update)
	visits.Post("/:id/complete",     perm("visiting", "complete"), visitH.Complete)
	visits.Post("/:id/reschedule",   perm("visiting", "reschedule"), visitH.Reschedule)
	visits.Post("/:id/cancel",       perm("visiting", "cancel"), visitH.Cancel)

	// Customer visiting history (piggybacks on internet-accounts route)
	ia.Get("/:id/visits", perm("visiting", "view"), visitH.ByAccount)
